...
const char * const BABOCOIN_CONF_FILENAME = "babocoin.conf";
const char * const BABOCOIN_PID_FILENAME = "babocoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "babocoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Babocoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Babocoin
    // Mac: ~/Library/Application Support/Babocoin
    // Unix: ~/.babocoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Babocoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Babocoin";
#else
    // Unix
    return pathRet / ".babocoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BABOCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BABOCOIN_PID_FILENAME)));
}
