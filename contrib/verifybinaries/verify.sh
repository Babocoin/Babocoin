...
###   babocoincore.org and babocoin.org and compares them.
...
WORKINGDIR="/tmp/babocoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://babocoincore.org"
HOST2="https://babocoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="babocoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'babocoin-',
...
if [ $? -ne 0 ]; then
   echo "babocoin.org failed to provide signature file, but babocoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "babocoin.org and babocoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Babocoin Core binary release signing key installed?"
   fi
